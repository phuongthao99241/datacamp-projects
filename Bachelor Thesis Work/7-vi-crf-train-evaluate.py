...
# Split the dataset
train_data = pd.read_csv('processed_kaggle_dataset.csv', encoding='utf-8')
temp_data = pd.read_csv('processed_website_dataset.csv', encoding='utf-8')

sentence_ids = temp_data['sentence_id'].unique()
test_sentence_ids, dev_sentence_ids = train_test_split(sentence_ids, test_size=0.5, random_state=42)

test_data = temp_data[temp_data['sentence_id'].isin(test_sentence_ids)]
...
# Convert features for each dataset
...
with open('vi_crf_model.pkl', 'wb') as file:
    pickle.dump(final_crf_model, file)
